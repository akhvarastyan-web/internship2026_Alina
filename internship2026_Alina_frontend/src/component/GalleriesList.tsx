...
export const GalleriesList = () => {
  ...
  const galleries: GalleryListItem[] = data?.data || [];
  ...
  return (
    <section className="flex flex-col gap-10 p-4 lg:p-0">
      ...
      <div className="grid grid-cols-1 justify-items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {galleries.map((gallery) => (
          <div
            key={gallery.id}
            className="w-[235px] h-[287px] flex flex-col bg-white rounded shadow-sm border border-gray-100 overflow-hidden"
          >

            <div className="w-[235px] h-[235px] grid grid-cols-3 grid-rows-3 gap-[2px] bg-gray-100 shrink-0">
              {Array.from({ length: 9 }).map((_, index) => {
                const imgUrl = gallery.images[index];
                return (
            ...
            <div className="p-2 flex flex-col flex-1 justify-center overflow-hidden">
              <h3 className="text-sm font-bold text-gray-900 truncate">{gallery.title}</h3>
              {gallery.description && (
